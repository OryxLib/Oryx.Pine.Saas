...
{
    public class BaseIdentityClientsBaseController<T> : Controller
            where T : IdentityServer4.EntityFramework.Entities.Client, new()
    {
        DbContext dbContext;
        ModelMapper modelMapper;
        public BaseIdentityClientsBaseController(DbContext _dbContext)
        {
        ...
        public async Task<IActionResult> List(string query, int page = 1, int limit = 15)
        {
            ...
            try
            {
                ...
                data = await sqlQuery.OrderBy("Created").Skip((page - 1) * limit).Take(limit).ToDynamicListAsync();
            }
            catch (Exception exc)
            {
                msg = exc.Message;
