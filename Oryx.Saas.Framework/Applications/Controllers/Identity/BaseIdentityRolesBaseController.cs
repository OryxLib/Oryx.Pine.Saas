...
{
    public class BaseIdentityRolesBaseController<T> : Controller
            where T : IdentityRole, new()
    {
        DbContext dbContext;
        ModelMapper modelMapper;
        public BaseIdentityRolesBaseController(DbContext _dbContext)
        {
        ...
        public async Task<IActionResult> AddOrUpdate(T userInfoModel)
        {
            ...
            {
                if (string.IsNullOrEmpty(userInfoModel.Id))
                {
                    userInfoModel.Id = Guid.NewGuid().ToString();
